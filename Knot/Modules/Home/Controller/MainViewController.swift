...
class MainViewController: BaseViewController {
    ...
    private let monitorQueue = DispatchQueue(label: "com.knot.network.monitor")
...
//        return
    ...
    func handleNetworkChange(_ path: Network.NWPath) {
    ...
    @objc func currentTaskDidClick(){
        if let task = CaptureTask.findFirst(orders: ["id":false]) {
    ...
    @objc func updateData(){
        if vpnStatus == .connected {
            do{
                currentTask = CaptureTask.getLast(false)
            }catch{
                print("读取最后一个报错了！")
            }
            if !currentIsWifi {
    ...
    @objc func showLog(){
        guard let directory = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: GROUPNAME) else { return }
...
extension MainViewController: HistoryViewDelegate,ProxyViewDelegate {
    ...
    func proxyServerChange(type: ServerType, value: Bool) {
        ...
        ipc?.passMessage(dic.toJson(), identifier: TaskConfigDidChanged)
//        ZKProgressHUD.show()
    ...
    func taskDidClick(task: CaptureTask) {
        navigationController?.pushViewController(SessionListViewController(task: task), animated: true)
    }
